...
SET "FRAME_COUNTER=0"
CALL :SET_ROWS 64
FOR /L %%A IN (0, 1, 127) DO SET "SYMODULE_PAT%%A="
...
:INSERT_FRAME #
SET "TEMPVARI00=NONE"
FOR /L %%A IN (127, -1, 0) DO IF NOT DEFINED SYMODULE_PAT%%A SET "TEMPVARI00=%%A
...
:DRAW_FRAME_EDITOR
IF !CURSOR_FRTAB! EQU 1 (
	SET "TEMPVARI00=[7m<INS>[27m  <MOV>  <SET>  <DUP>  <REM>  <X>"
) ELSE IF !CURSOR_FRTAB! EQU 2 (
	SET "TEMPVARI00=<INS>  [7m<MOV>[27m  <SET>  <DUP>  <REM>  <X>"
) ELSE IF !CURSOR_FRTAB! EQU 3 (
	SET "TEMPVARI00=<INS>  <MOV>  [7m<SET>[27m  <DUP>  <REM>  <X>"
) ELSE IF !CURSOR_FRTAB! EQU 4 (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  [7m<DUP>[27m  <REM>  <X>"
) ELSE IF !CURSOR_FRTAB! EQU 5 (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  <DUP>  [7m<REM>[27m  <X>"
) ELSE IF !CURSOR_FRTAB! EQU 6 (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  <DUP>  <REM>  [7m<X>[27m"
) ELSE (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  <DUP>  <REM>  <X>"
)
...
FOR /L %%A IN (0, 1, 127) DO IF "!SYMODULE_PAT%%A!"=="!BLANKTABLE!" (
	SET "TEMPVARI00"
	FOR %%B IN (!SYMODULE_FRAME!) DO IF %%B EQU %%A SET "TEMPVARI00=1"
	IF !TEMPVARI00! NEQ 0 SET "SYMODULE_PAT%%A="
)
GOTO :EOF
...
REM REM 00
REM :DELETE_PATTERN #
REM IF NOT DEFINED SYMODULE_PAT%~1 GOTO :EOF
REM FOR /L %%A IN (1, 1, !FRAME_COUNTER!) DO (
	REM SET /A "TEMPVARI00=%%A - 1"
	REM IF %%A GEQ %~1 SET "SYMODULE_PAT!TEMPVARI00!=!SYMODULE_PAT%%A!"
REM )
REM GOTO :EOF
...
SET "SYMODULE_PAT128="
IF !FRAME_COUNTER! EQU 0 (
	SET "SYMODULE_ROWS=%~1"
	GOTO :EOF
)
FOR /L %%A IN (0, 1, 127) DO IF DEFINED "SYMODULE_PAT%%A" (
