...
IF !TRACKER_MODE! EQU 0 (
	...
	IF !ERRORLEVEL! EQU 38 SET /A "CURSOR_FRTAB+=16"
	IF !ERRORLEVEL! EQU 40 SET /A "CURSOR_FRTAB-=16"
...
CALL :RESET
REM -- Should do this because APPEND_FRAME tries to create a blank pattern at FRAME_COUNTER
SET "FRAME_COUNTER=0"
FOR /L %%A IN (0, 1, 9) DO CALL :APPEND_FRAME
GOTO :EOF
...
:APPEND_FRAME
CALL :CLEAR_PATTERN !FRAME_COUNTER!
SET "SYMODULE_FRAME=!SYMODULE_FRAME!!FRAME_COUNTER!;"
...
FOR %%A IN (!SYMODULE_FRAME!) DO (
	IF !TEMPVARI01! NEQ %~1 SET "TEMPVARI01=!TEMPVARI01!%%A;"
	SET /A "TEMPVARI00+=1"
)
SET "SYMODULE_FRAME=!TEMPVARI01!"
IF !CURSOR_FRAME! EQU %~1 CALL :MOVE_TO_FRAME "PREV"
CALL :UPDATE_FRAME_COUNTER
GOTO :EOF

REM 00 01
:MODIFY_FRAME FROM TO
SET "TEMPVARI00="
FOR %%A IN (!SYMODULE_FRAME!) DO (
	IF !TEMPVARI01! EQU %~1 (
		SET "TEMPVARI01=!TEMPVARI01!%~1;"
	) ELSE (
...
IF !CURSOR_FRTAB! EQU -1 (
	SET "TEMPVARI00=[7m<INS>[27m  <MOV>  <SET>  <DUP>  <REM>"
) ELSE IF !CURSOR_FRTAB! EQU -2 (
	SET "TEMPVARI00=<INS>  [7m<MOV>[27m  <SET>  <DUP>  <REM>"
) ELSE IF !CURSOR_FRTAB! EQU -3 (
	SET "TEMPVARI00=<INS>  <MOV>  [7m<SET>[27m  <DUP>  <REM>"
) ELSE IF !CURSOR_FRTAB! EQU -4 (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  [7m<DUP>[27m  <REM>"
) ELSE IF !CURSOR_FRTAB! EQU -1 (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  <DUP>  [7m<REM>[27m"
) ELSE (
	SET "TEMPVARI00=<INS>  <MOV>  <SET>  <DUP>  <REM>"
)
...
FOR /L %%A IN (0, 1, !SYMODULE_ROWS!) DO (
	FOR /L %%B IN (0, 1, 5) DO SET "SYMODULE_PAT%~1=!SYMODULE_PAT%~1!C_%~1_________"
...
FOR /L %%A IN (1, 1, !FRAME_COUNTER!) DO (
...
ECHO [%~2;%~1H[0m[38;2;!TEMPVARI00!;!TEMPVARI01!;!TEMPVARI02!m________________\[38;2;!SYCONFIG_COLOR_LGTXT!mSAEYAHN[38;2;!TEMPVARI00!;!TEMPVARI01!;!TEMPVARI02!m/________________ 
...
SET /A "TEMPVARI00=%~1 + 7,TEMPVARI01=%~2 + 7"
...
FOR /F "TOKENS=*" %%A IN ('SET SYCONFIG') DO (
	SET "TEMPVARI00=%%A"
	ECHO %%A>>"!SY_APPCONFIGPATH!\config.txt"
)
GOTO :EOF
